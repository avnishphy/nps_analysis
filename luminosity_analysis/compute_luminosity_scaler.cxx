...
static double compute_median_expected_current(
  ...
  for (const auto &path : files) {
    ...
    for (Long64_t i = 0; i < n; ++i) {
      tsh->GetEntry(i);
      if (std::isfinite(bcmCurrent)) currents.push_back(bcmCurrent);
    }
  }
...
static RunResult analyze_run(
  ...
  // Determine which TRIG to use based on prescale_token
  int trig_num = extract_trig_number(r.prescale_token);
  if (trig_num < 0) trig_num = 4; // fallback to 4 if not found
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < total_runs; ++i) {
    ...
    // Priority: prescale_lookup > default_ps
    ...
    rr.ps_factor = ps;
    // Set which_TRIG for output (redundant if already set in analyze_run, but safe)
    int trig_num = extract_trig_number(prescale_token);
    if (trig_num < 0) trig_num = 4;
    char htrig_branch[64];
    snprintf(htrig_branch, sizeof(htrig_branch), "H.hTRIG%d.scaler", trig_num);
  ...
  if (!out) {
    ...
    out << "run,n_files,expected_current_uA,avg_current_uA,charge_uC,"
      ...
      << "cltp_livetime_tdc,tlt_livetime_edtm,negative_dt_intervals,"
