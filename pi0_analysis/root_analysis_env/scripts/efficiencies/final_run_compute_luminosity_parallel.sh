#!/usr/bin/env bash
# final_run_compute_luminosity_parallel.sh
# Wrapper to run final_compute_luminosity_standalone.cxx in parallel for multiple runs

usage() {
  cat <<'EOF'
Usage:
  final_run_compute_luminosity_parallel.sh --runs <run tokens...> [options]
...
Options:
  --binary <path>                 Path to compute executable
                                  (default: ./final_compute_luminosity_standalone)
...
binary="$script_dir/final_compute_luminosity_standalone"
out_csv="$script_dir/final_luminosity_results_parallel.csv"
root_dir=""
db_path=""
default_ps=""
current_window=""
...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/final_luminosity_parallel.XXXXXX")"
cleanup() {
