// ============================================================================
// Author: Original Fortran code by P. Bosted; Adapted by Avnish Singh + ChatGPT
// Date: 2025-11-04
// ============================================================================
...
void pi0_analysis_peter(const TString &skimDir_in="output/skimmed/",
    ...
    for (int run : runs) {
        ...
        for (Long64_t evnt = 0; evnt < nentries; ++evnt) {
            ...
            if (HgtrTh > -1 && HgtrTh < 1 && HgtrPh > -1 && HgtrPh < 1 && hcaletot > 0.001) {
                    ...
                    if (nclust > 1 && mbest > 0.05 &&
                        ...
                        for (int j = 0; j < 2; ++j) {
                            ...
                            for (int b = 0; b < N_BLOCKS; ++b) {
                                ...
                                // Only consider blocks that belong to this cluster and have positive energy
                                if (icl == nn && std::isfinite(block_e[b]) && block_e[b] > 0.0 && std::isfinite(block_t[b])) {
                                    if (f2) fprintf(f2,"%4d %3d %8.4f %8.1f \n", b, icl, block_e[b], block_t[b]);

                                    // Only process "dominant" blocks (e.g. seed blocks)
                                    if (block_e[b] > 0.3 * etot) {

                                        // we store only if index lies inside 0..19
                                        int jj = static_cast<int>(block_t[b] - 140.0); // truncates toward zero
                                        if (jj >= 0 && jj < 20) {
                                            blktimeh[b][jj]++;
                                        }

                                        // --- mass bin index  ---
                                        // map mbest into 0..19 for mass in [0.115,0.155)
                                        int jj2 = static_cast<int>((mbest - 0.115) * 20.0 / 0.040); // truncates
                                        if (jj2 >= 0 && jj2 < 20) {
                                            blkmassh[b][jj2]++;
                                        }
                                    }
