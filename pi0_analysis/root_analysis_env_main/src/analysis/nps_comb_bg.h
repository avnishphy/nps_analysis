                                                         ...
                                                         bool draw = true)
{
    ...
    if (draw) {
        ...
        double bg_max = 0.0;
        for (int b = 1; b <= nbins; ++b) {
            const double x = h_coin_bgsub->GetXaxis()->GetBinCenter(b);
            bg_max = std::max(bg_max, f_bg->Eval(x));
        }
        double pad1_ymax = std::max(h_coin_bgsub->GetMaximum(), bg_max);
        if (!std::isfinite(pad1_ymax) || pad1_ymax <= 0.0) pad1_ymax = 1.0;
        h_coin_bgsub->SetMinimum(0.0);
        h_coin_bgsub->SetMaximum(1.20 * pad1_ymax);
        ...
        double pad2_ymax = std::max(h_before->GetMaximum(), h_final->GetMaximum());
        if (!std::isfinite(pad2_ymax) || pad2_ymax <= 0.0) pad2_ymax = 1.0;
        h_before->SetMinimum(0.0);
        h_before->SetMaximum(1.20 * pad2_ymax);
        h_before->Draw("HIST");
        ...
        double zoom_yhi = std::max(h_final->GetMaximum(), f_sig.GetMaximum(signal_lo, signal_hi));
        if (!std::isfinite(zoom_yhi) || zoom_yhi <= zoom_ylo) zoom_yhi = zoom_ylo + 1.0;
        zoom_yhi *= 1.20;
