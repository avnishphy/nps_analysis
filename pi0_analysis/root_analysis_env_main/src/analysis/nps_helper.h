                              ...
                              bool draw = true, const TString &outPlotDir = "", int run = -1)
{
    ...
    if (draw) {
        ...
        double y_plot_max = h->GetMaximum();
        if (!std::isfinite(y_plot_max) || y_plot_max <= 0.0) y_plot_max = 1.0;
        h->SetMinimum(0.0);
        h->SetMaximum(1.20 * y_plot_max);
        ...
        const double fit_ymax = fdisplay->GetMaximum(h->GetXaxis()->GetXmin(), h->GetXaxis()->GetXmax());
        if (std::isfinite(fit_ymax) && fit_ymax > y_plot_max) {
            h->SetMaximum(1.20 * fit_ymax);
        }
        fdisplay->Draw("same");
                                  ...
                                  double gaus_mu = kPi0Mass_GeV, double gaus_sigma = 0.008)
{
    ...
    if (draw) {
        ...
        double y_plot_max = std::max(h_coin->GetMaximum(), h_coin_bgsub->GetMaximum());
        y_plot_max = std::max(y_plot_max, f_lin1_final->GetMaximum(lin1_lo, lin1_hi));
        y_plot_max = std::max(y_plot_max, f_gaus_final->GetMaximum(gaus_lo, gaus_hi));
        y_plot_max = std::max(y_plot_max, f_lin2_final->GetMaximum(lin2_lo, lin2_hi));
        if (!std::isfinite(y_plot_max) || y_plot_max <= 0.0) y_plot_max = 1.0;

        h_coin->SetLineColor(kBlack); h_coin->SetLineWidth(1);
        h_coin->SetMinimum(0.0);
        h_coin->SetMaximum(1.20 * y_plot_max);
        h_coin->Draw("E");
