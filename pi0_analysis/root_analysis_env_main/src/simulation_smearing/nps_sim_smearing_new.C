    ...
    //
    //   ./nps_sim_smearing_new /w/hallc-scshelf2102/nps/singhav/nps_analysis/pi0_analysis/root_analysis_env/output/plots/x60_4b/production_wfpi0/combined_branches_LH2_wfpi0.root physics /w/hallc-scshelf2102/nps/singhav/nps_analysis/pi0_analysis/root_analysis_env/output/plots/x60_4b/production_wfpi0/simc_pi0_analysis_output.root simulation out_smear.root 11 16 -24 28 -34 34 0.2 50 --z-nps-cm 407 --nps-angle-deg 17.51
    // 
    // Output files:
    ...
    #include "../analysis/nps_comb_bg.h"
    #include "../analysis/nps_mmiss_cor.h"
    #include "../analysis/nps_hms_track_eff.h"
    ...
    namespace Config {
        ...
        double BEAM_ENERGY = std::numeric_limits<double>::quiet_NaN();
        ...
        inline bool fit_objective_uses_mmiss() {
            return (W_MMISS != 0.0) &&
                   (ENERGY_SMEARING_HISTOGRAM == HIST_MMISS_ONLY ||
                    ENERGY_SMEARING_HISTOGRAM == HIST_BOTH);
        }

        inline bool use_mpi0_histogram() {
            return W_MPI0 != 0.0;
        }

        inline bool use_mmiss_histogram() {
            return W_MMISS != 0.0;
        }

        inline bool use_mpgg2_histogram() {
            return W_MPGG2 != 0.0;
        }
        ...
        inline void print_configuration_summary() {
            ...
            cout << "Beam energy: " << BEAM_ENERGY << " GeV\n";
            cout << "NPS geometry: z=" << NPS_Z_NPS_CM
    ...
    inline void apply_mgg_linear_pair_energy_correction(double &E1,
        ...
        const double mgg = nps::invariant_mass_pi0(E1, E2, x1, x2, y1, y2,
                                                   Config::NPS_Z_NPS_CM);
    ...
    inline void fillSmearedHistogramsAtParams(const vector<ClusterPair> &simEvents,
                                            ...
                                            double res_C,
                                            bool fill_mpi0 = true,
                                            bool fill_mmiss = true,
                                            bool fill_mpgg2 = true);
    ...
    ObjectiveBreakdown eval_objective_breakdown(double mu_a, double mu_b, double mu_c,
        ...
        fillUnsmearedHistogramsForNormalization(simEvents, hnorm_mpi0, hnorm_mmiss, hnorm_mpgg2,
                                                p_e_scale,
                                                Config::use_mpi0_histogram(),
                                                Config::use_mmiss_histogram(),
                                                Config::use_mpgg2_histogram());
    ...
    double eval_chi2_mpi0_only(double mu_a, double mu_b, double mu_c,
                            ...
                            int Nsmear,
                            double res_A, double res_B, double res_C) {
        if (!Config::use_mpi0_histogram()) return 0.0;
    ...
    double eval_chi2_mmiss_only(double mu_a, double mu_b, double mu_c,
                                ...
                                int Nsmear,
                                double res_A, double res_B, double res_C) {
        if (!Config::use_mmiss_histogram()) return 0.0;
    ...
    double eval_chi2_mpgg2_only(double mu_a, double mu_b, double mu_c,
                                ...
                                int Nsmear,
                                double res_A, double res_B, double res_C) {
        if (!Config::use_mpgg2_histogram()) return 0.0;
    ...
    int main(int argc, char** argv) {
        if (argc < 13) {
            cout << "Usage: " << argv[0] << " data.root dataTree sim.root simTree out.root nx ny x_min x_max y_min y_max overlap_frac [Nsmear] --beam-energy-gev <GeV> --z-nps-cm <cm> (--theta-nps-deg <deg> | --nps-angle-deg <deg>) [--input-preview-pdf <path> --confirm-inputs]\n";
            cout << "Example: " << argv[0] << " data.root dataTree sim.root simTree out.root 8 8 -20 20 -20 20 0.2 20 --beam-energy-gev 10.538 --z-nps-cm 407 --nps-angle-deg 17.51 --input-preview-pdf input_histograms_prefit.pdf --confirm-inputs\n";
            return 1;
        }
        ...
        if (argi < argc && string(argv[argi]).rfind("--", 0) != 0) {
            Nsmear = atoi(argv[argi]);
            ++argi;
        }

        auto parse_required_double = [&](const string &flag, const char *value_text, double &out) {
            if (!value_text || value_text[0] == '\0') {
                cerr << "ERROR: " << flag << " requires a value." << endl;
                return false;
            }
            char *end = nullptr;
            const double parsed = std::strtod(value_text, &end);
            if (!end || *end != '\0' || !std::isfinite(parsed)) {
                cerr << "ERROR: " << flag << " requires a finite numeric value (got '" << value_text << "')." << endl;
                return false;
            }
            out = parsed;
            return true;
        ...
        bool have_z_nps = false;
        bool have_theta_nps = false;
        bool have_physical_nps_angle = false;
        bool have_beam_energy = false;
        string input_preview_pdf;
        bool confirm_inputs = false;
        while (argi < argc) {
            string flag = argv[argi++];
            if (flag == "--beam-energy-gev" || flag == "--beam-energy") {
                if (argi >= argc || !parse_required_double(flag, argv[argi++], Config::BEAM_ENERGY)) return 1;
                have_beam_energy = true;
            } else if (flag == "--z-nps-cm") {
                if (argi >= argc || !parse_required_double(flag, argv[argi++], Config::NPS_Z_NPS_CM)) return 1;
                have_z_nps = true;
            } else if (flag == "--theta-nps-deg") {
                if (have_physical_nps_angle) {
                    cerr << "ERROR: use either --theta-nps-deg or --nps-angle-deg, not both." << endl;
                    return 1;
                }
                if (argi >= argc || !parse_required_double(flag, argv[argi++], Config::NPS_THETA_NPS_DEG)) return 1;
                have_theta_nps = true;
            } else if (flag == "--nps-angle-deg") {
                if (have_theta_nps) {
                    cerr << "ERROR: use either --theta-nps-deg or --nps-angle-deg, not both." << endl;
                    return 1;
                }
                double physical_angle = std::numeric_limits<double>::quiet_NaN();
                if (argi >= argc || !parse_required_double(flag, argv[argi++], physical_angle)) return 1;
                Config::NPS_THETA_NPS_DEG = -physical_angle;
                have_physical_nps_angle = true;
            } else if (flag == "--input-preview-pdf") {
                if (argi >= argc || argv[argi][0] == '\0') {
                    cerr << "ERROR: --input-preview-pdf requires a path." << endl;
                    return 1;
                }
                input_preview_pdf = argv[argi++];
            } else if (flag == "--confirm-inputs") {
                confirm_inputs = true;
            } else {
                cerr << "ERROR: unknown option '" << flag << "'." << endl;
                cerr << "Usage: " << argv[0] << " data.root dataTree sim.root simTree out.root nx ny x_min x_max y_min y_max overlap_frac [Nsmear] --beam-energy-gev <GeV> --z-nps-cm <cm> (--theta-nps-deg <deg> | --nps-angle-deg <deg>) [--input-preview-pdf <path> --confirm-inputs]" << endl;
                return 1;
            }
        }

        if (!have_beam_energy || !(Config::BEAM_ENERGY > 0.0)) {
            cerr << "ERROR: explicit beam energy is required. Pass --beam-energy-gev <positive_GeV>." << endl;
            return 1;
        }
        if (!have_z_nps || !(Config::NPS_Z_NPS_CM > 0.0)) {
            cerr << "ERROR: explicit NPS z distance is required. Pass --z-nps-cm <positive_cm>." << endl;
            return 1;
        }
        if (!have_theta_nps && !have_physical_nps_angle) {
            cerr << "ERROR: explicit NPS angle is required. Pass --theta-nps-deg <signed_NPS_to_Hall_deg> or --nps-angle-deg <physical_deg>." << endl;
            return 1;
        }
        if (confirm_inputs && input_preview_pdf.empty()) {
            cerr << "ERROR: --confirm-inputs requires --input-preview-pdf <path>." << endl;
            return 1;
        }
        ...
        for (Long64_t i=0;i<ndata;++i) {
            ...
            PhotonMomentum dp1 = computePhotonMomentum(d_cluster_e_1_mpgg2, d_cluster_x_1, d_cluster_y_1, Config::NPS_Z_NPS_CM);
            PhotonMomentum dp2 = computePhotonMomentum(d_cluster_e_2_mpgg2, d_cluster_x_2, d_cluster_y_2, Config::NPS_Z_NPS_CM);
        ...
        // Before any optimization, show exactly which weighted data and nominal
        // simulation distributions will seed the fit. Diagnostics always include
        // all observables, even when an observable has zero objective weight.
        if (!input_preview_pdf.empty()) {
            auto normalize_preview_sim = [](TH1D &hsim, const TH1D &hdata) {
                const double sim_integral = hsim.Integral();
                const double data_integral = hdata.Integral();
                if (sim_integral > 0.0 && data_integral > 0.0) {
                    hsim.Scale(data_integral / sim_integral);
                }
            };

            auto draw_input_preview_pad = [](const TH1D &hdata, const TH1D &hsim,
                                             const char *x_title, const char *title) {
                TH1D *data_draw = dynamic_cast<TH1D*>(hdata.Clone());
                TH1D *sim_draw = dynamic_cast<TH1D*>(hsim.Clone());
                data_draw->SetDirectory(nullptr);
                sim_draw->SetDirectory(nullptr);
                data_draw->SetBit(kCanDelete);
                sim_draw->SetBit(kCanDelete);
                data_draw->SetTitle(Form("%s;%s;Weighted counts", title, x_title));
                data_draw->SetLineColor(kBlack);
                data_draw->SetMarkerColor(kBlack);
                data_draw->SetMarkerStyle(20);
                data_draw->SetMarkerSize(0.65);
                sim_draw->SetLineColor(kRed + 1);
                sim_draw->SetLineWidth(2);
                const double ymax = 1.20 * max(data_draw->GetMaximum(), sim_draw->GetMaximum());
                data_draw->SetMinimum(0.0);
                data_draw->SetMaximum(ymax > 0.0 ? ymax : 1.0);
                data_draw->Draw("E1");
                sim_draw->Draw("HIST SAME");
                data_draw->Draw("E1 SAME");
                TLegend *legend = new TLegend(0.58, 0.75, 0.89, 0.89);
                legend->SetBit(kCanDelete);
                legend->SetBorderSize(1);
                legend->SetFillColor(kWhite);
                legend->AddEntry(data_draw, "Data", "lep");
                legend->AddEntry(sim_draw, "Nominal simulation", "l");
                legend->Draw();
            };

            auto fill_preview_sim = [&](const vector<ClusterPair> &events,
                                        TH1D &hmgg, TH1D &hmmiss, TH1D &hmpgg2) {
                fillUnsmearedHistogramsForNormalization(events, hmgg, hmmiss, hmpgg2,
                                                        1.0, true, true, true);
            };

            TCanvas preview_canvas("c_smearing_input_preview", "Smearing input preview", 1500, 1000);
            preview_canvas.Print((input_preview_pdf + "[").c_str());

            auto print_preview_page = [&](const string &page_title,
                                          const vector<ClusterPair> &events,
                                          const TH1D &data_mgg,
                                          const TH1D &data_mmiss,
                                          const TH1D &data_mpgg2,
                                          const string &selection_note) {
                static int preview_page = 0;
                ++preview_page;
                TH1D sim_mgg(Form("h_preview_mgg_%d", preview_page), "",
                             Config::MGGAMMA_NBINS, Config::MGGAMMA_MIN, Config::MGGAMMA_MAX);
                TH1D sim_mmiss(Form("h_preview_mmiss_%d", preview_page), "",
                               Config::MMISS_NBINS, Config::MMISS_MIN, Config::MMISS_MAX);
                TH1D sim_mpgg2(Form("h_preview_mpgg2_%d", preview_page), "",
                               Config::MPGG2_NBINS, Config::MPGG2_MIN, Config::MPGG2_MAX);
                sim_mgg.Sumw2();
                sim_mmiss.Sumw2();
                sim_mpgg2.Sumw2();
                fill_preview_sim(events, sim_mgg, sim_mmiss, sim_mpgg2);
                normalize_preview_sim(sim_mgg, data_mgg);
                normalize_preview_sim(sim_mmiss, data_mmiss);
                normalize_preview_sim(sim_mpgg2, data_mpgg2);

                preview_canvas.Clear();
                preview_canvas.Divide(2, 2);
                preview_canvas.cd(1); gPad->SetLeftMargin(0.13); gPad->SetBottomMargin(0.12);
                draw_input_preview_pad(data_mgg, sim_mgg,
                                       "M_{#gamma#gamma} [GeV/c^{2}]", "M_{#gamma#gamma}");
                preview_canvas.cd(2); gPad->SetLeftMargin(0.13); gPad->SetBottomMargin(0.12);
                draw_input_preview_pad(data_mmiss, sim_mmiss,
                                       "M_{miss} [GeV/c^{2}]", "M_{miss}");
                preview_canvas.cd(3); gPad->SetLeftMargin(0.13); gPad->SetBottomMargin(0.12);
                draw_input_preview_pad(data_mpgg2, sim_mpgg2,
                                       "(p_{target}+#gamma#gamma)^{2} [GeV^{2}]",
                                       "M_{p#gamma#gamma}^{2}");
                preview_canvas.cd(4);
                TPaveText *info = new TPaveText(0.04, 0.05, 0.96, 0.95, "NDC");
                info->SetBit(kCanDelete);
                info->SetBorderSize(1);
                info->SetFillColor(kWhite);
                info->SetTextAlign(12);
                info->SetTextSize(0.034);
                info->AddText(page_title.c_str());
                info->AddText(selection_note.c_str());
                info->AddText(Form("data entries: Mgg=%.0f  Mmiss=%.0f  Mpgg2=%.0f",
                                   data_mgg.GetEntries(), data_mmiss.GetEntries(), data_mpgg2.GetEntries()));
                info->AddText(Form("sim selected events: %zu", events.size()));
                info->AddText(Form("weights: W_MPI0=%.4g  W_MMISS=%.4g  W_MPGG2=%.4g",
                                   Config::W_MPI0, Config::W_MMISS, Config::W_MPGG2));
                info->AddText(Form("objective use: Mgg=%s  Mmiss=%s  Mpgg2=%s",
                                   Config::use_mpi0_histogram() ? "yes" : "no",
                                   Config::use_mmiss_histogram() ? "yes" : "no",
                                   Config::use_mpgg2_histogram() ? "yes" : "no"));
                info->AddText(Form("NPS z=%.6g cm  theta(NPS->Hall)=%.6g deg",
                                   Config::NPS_Z_NPS_CM, Config::NPS_THETA_NPS_DEG));
                info->AddText(Form("simulation Mmiss beam energy=%.6g GeV", Config::BEAM_ENERGY));
                info->AddText("data Mmiss source: precomputed mmiss_all branch");
                info->AddText("Each nominal simulation histogram is normalized to its data integral.");
                info->Draw();
                preview_canvas.Modified();
                preview_canvas.Update();
                preview_canvas.Print(input_preview_pdf.c_str());
            };

            print_preview_page("Global fit input preview", sim_events_summary,
                               hdata_summary_mpi0, hdata_summary_mmiss, hdata_summary_mpgg2,
                               "Selection: both photons inside configured geometry");
            for (int is = 0; is < nsec; ++is) {
                print_preview_page("Section " + sections[is].name(), sim_events_per_section[is],
                                   *hdata_sec[is], *hdata_mmiss_sec[is], *hdata_mpgg2_sec[is],
                                   Form("x=[%.3g, %.3g] cm, y=[%.3g, %.3g] cm",
                                        sections[is].xlo, sections[is].xhi,
                                        sections[is].ylo, sections[is].yhi));
            }
            preview_canvas.Print((input_preview_pdf + "]").c_str());

            cout << "\n==== Input histogram verification required ====\n"
                 << "Preview PDF: " << input_preview_pdf << "\n"
                 << "It contains global and per-section data/nominal-simulation overlays for "
                 << "Mgg, Mmiss, and Mpgg2.\n";
            if (confirm_inputs) {
                cout << "Verify the plots and configuration above. Start optimization? [y/N] " << flush;
                string response;
                if (!getline(cin, response) || !(response == "y" || response == "Y" ||
                                                  response == "yes" || response == "YES" ||
                                                  response == "Yes")) {
                    cout << "Optimization cancelled; no fit output was written." << endl;
                    return 20;
                }
                cout << "Input histograms approved. Starting optimization.\n" << endl;
            }
        }
        ...
        string source_path_for_hash = "src/simulation_smearing/nps_sim_smearing_new.C";
        ...
        auto plot_iteration_candidate_histograms =
            ...
            ObjectiveBreakdown plotted_breakdown;
            plotted_breakdown.mpi0 = computeChi2MetricsFromHist(hs_mgg, hdata_summary_mpi0);
            plotted_breakdown.mmiss = computeChi2MetricsFromHist(hs_mmiss, hdata_summary_mmiss);
            plotted_breakdown.mpgg2 = computeChi2MetricsFromHist(hs_mpgg2, hdata_summary_mpgg2);
            ...
            txt->AddText(Form("chi2 Mgg=%.6g  Mmiss=%.6g  Mpgg2=%.6g",
                              plotted_breakdown.mpi0.chi2,
                              plotted_breakdown.mmiss.chi2,
                              plotted_breakdown.mpgg2.chi2));
            ...
            for (int is = 0; is < nsec; ++is) {
                if (!src_success[is]) continue;
                if ((Config::use_mpi0_histogram() && hdata_sec[is]->Integral() <= 0.0) ||
                    (Config::use_mmiss_histogram() && hdata_mmiss_sec[is]->Integral() <= 0.0) ||
                    (Config::use_mpgg2_histogram() && hdata_mpgg2_sec[is]->Integral() <= 0.0)) continue;
        ...
        auto fit_section_fast_refine = [&](const vector<ClusterPair> &simEvents,
            ...
            int n_bins_chi2 = 0;
            if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MPI0_ONLY) {
                n_bins_chi2 = Config::use_mpi0_histogram() ? countInformativeBinsData(hdata_mpi0) : 0;
            } else if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MMISS_ONLY) {
                n_bins_chi2 = Config::use_mmiss_histogram() ? countInformativeBinsData(hdata_mmiss) : 0;
            } else if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MPGG2_ONLY) {
                n_bins_chi2 = Config::use_mpgg2_histogram() ? countInformativeBinsData(hdata_mpgg2) : 0;
            } else {
                n_bins_chi2 = (Config::use_mpi0_histogram() ? countInformativeBinsData(hdata_mpi0) : 0)
                            + (Config::use_mmiss_histogram() ? countInformativeBinsData(hdata_mmiss) : 0)
                            + (Config::use_mpgg2_histogram() ? countInformativeBinsData(hdata_mpgg2) : 0);
            }
            int n_norm_hists = 0;
            if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MPI0_ONLY) {
                n_norm_hists = Config::use_mpi0_histogram() ? 1 : 0;
            } else if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MMISS_ONLY) {
                n_norm_hists = Config::use_mmiss_histogram() ? 1 : 0;
            } else if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MPGG2_ONLY) {
                n_norm_hists = Config::use_mpgg2_histogram() ? 1 : 0;
            } else {
                n_norm_hists = (Config::use_mpi0_histogram() ? 1 : 0)
                             + (Config::use_mmiss_histogram() ? 1 : 0)
                             + (Config::use_mpgg2_histogram() ? 1 : 0);
            }
        ...
        if (Config::ENABLE_GLOBAL_PREFIT_SEED) {
            ...
            if (data_global_prefit_selected >= Config::MIN_EVENTS_PER_SECTION &&
                ...
                (!Config::use_mpi0_histogram() || hdata_summary_mpi0.Integral() > 0.0) &&
                (!Config::use_mmiss_histogram() || hdata_summary_mmiss.Integral() > 0.0) &&
                (!Config::use_mpgg2_histogram() || hdata_summary_mpgg2.Integral() > 0.0) &&
        ...
        auto selected_objective_from_breakdown = [&](const ObjectiveBreakdown &bd) {
            if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MPI0_ONLY) {
                return Config::use_mpi0_histogram() ? bd.mpi0.chi2 : 0.0;
            }
            if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MMISS_ONLY) {
                return Config::use_mmiss_histogram() ? bd.mmiss.chi2 : 0.0;
            }
            if (Config::ENERGY_SMEARING_HISTOGRAM == Config::HIST_MPGG2_ONLY) {
                return Config::use_mpgg2_histogram() ? bd.mpgg2.chi2 : 0.0;
            }
        ...
        for (int is = 0; is < nsec; ++is) {
            ...
            for (const auto &ev : sim_events_per_section[is]) {
                if (ev.e1 <= 0 || ev.e2 <= 0) continue;
                PhotonMomentum p1u = computePhotonMomentum(ev.e1, ev.x1, ev.y1, Config::NPS_Z_NPS_CM);
                PhotonMomentum p2u = computePhotonMomentum(ev.e2, ev.x2, ev.y2, Config::NPS_Z_NPS_CM);
                double mass_u = computeInvariantMass(ev.e1, p1u.px, p1u.py, p1u.pz,
                                                    ev.e2, p2u.px, p2u.py, p2u.pz);
            ...
            const double z_nps_diag = Config::NPS_Z_NPS_CM;
            ...
            for (const auto &ev : sim_events_per_section[is]) {
                ...
                PhotonMomentum p1u_mpgg2 = computePhotonMomentum(E1u, ev.x1, ev.y1, z_nps_diag);
                PhotonMomentum p2u_mpgg2 = computePhotonMomentum(E2u, ev.x2, ev.y2, z_nps_diag);
                hunsmeared_mpgg2.Fill(computeTargetPlusDiphotonMass2(E1u, p1u_mpgg2, E2u, p2u_mpgg2), ev.weight);
            }
